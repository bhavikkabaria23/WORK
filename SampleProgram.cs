        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                    ...
                    "v9.0"))
                {
                    ...
                    //string pdfFile = @"D:\nopCommerce_4.20_changelog.pdf";
                    //byte[] pdfBytes = System.IO.File.ReadAllBytes(pdfFile);
                    //string pdfBase64 = Convert.ToBase64String(pdfBytes);

                    //annotation.Add("subject", "Test");
                    //annotation.Add("notetext", "Testing Prompt");
                    //annotation.Add("objectid_new_merchantboarding@odata.bind", "/new_merchantboardings(ADBC190F-CCD7-E911-A8DB-000D3A1617D5)");
                    //annotation.Add("objecttypecode", "new_merchantboarding");

                    ////Below code for attachment of Notes
                    //annotation.Add("isdocument", true);
                    //annotation.Add("documentbody", pdfBase64);
                    //annotation.Add("mimetype", @"application/octet-stream");
                    //annotation.Add("filename", "Test.pdf");

                    //HttpRequestMessage createRequestNotes = new HttpRequestMessage(HttpMethod.Post, client.BaseAddress + "annotations");
                    //createRequestNotes.Content = new StringContent(annotation.ToString());
                    //createRequestNotes.Content.Headers.ContentType = MediaTypeHeaderValue.Parse("application/json");

                    //HttpResponseMessage createResponseNotes = client.SendAsync(createRequestNotes, HttpCompletionOption.ResponseHeadersRead).Result;
                    //if (createResponseNotes.IsSuccessStatusCode)
                    //{

                    //}
                    //else
                    //{
                    //    throw new Exception(string.Format("Failed to Post Records", createResponseNotes.ReasonPhrase));
                    //}

                    #endregion Note Creation

                    #region MerchantBoarding
                    JObject merchant = new JObject();
                    merchant.Add("new_whoprocessestheorder1", "1,2");
                    merchant.Add("new_merchantprogramtype1", "6,7");
                    merchant.Add("new_methodofmarketing1", "4,5");
                    merchant.Add("new_transactiontype1", "2");
                    merchant.Add("new_name", "MultiSelect Test");

                    HttpRequestMessage createrequest_merchant = new HttpRequestMessage(HttpMethod.Post, client.BaseAddress + "new_merchantboardings");
                    createrequest_merchant.Content = new StringContent(merchant.ToString());
                    createrequest_merchant.Content.Headers.ContentType = MediaTypeHeaderValue.Parse("application/json");

                    HttpResponseMessage createResponse_merchant = client.SendAsync(createrequest_merchant, HttpCompletionOption.ResponseHeadersRead).Result;
                    if (createResponse_merchant.IsSuccessStatusCode)
                    {

                    }
                    else
                    {
                        throw new Exception(string.Format("Failed to Post Records", createResponse_merchant.ReasonPhrase));
                    }

                    #endregion MerchantBoarding
