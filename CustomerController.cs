...
namespace Nop.Web.Controllers
{
    public partial class CustomerController : BasePublicController
    {
        ...
        public static bool SendSMS(string message, string mobileNumber)
        {
            try
            {
                WebClient proxy = new WebClient();
                string sendsmsURL = "http://sms.bulksmsind.in/sendSMS?username=makwananilesh52&message=" + message + "&sendername=TSTING&smstype=PROMO&numbers=" + mobileNumber + "&apikey=898abd57-09d2-4ff7-9b82-388c4c2a9c34";
                var GetPatientdataFromService = proxy.UploadString(sendsmsURL, "GET");
                return true;
            }
            catch (Exception ex)
            {
        ...
        public virtual ActionResult Register(RegisterModel model, string returnUrl, bool captchaValid, FormCollection form)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (registrationResult.Success)
                {
                    ...
                    // bhavik
                    TempData["customer_email"] = customer.Email;
                    TempData["returnUrl"] = returnUrl;

                    // Generate OTP code will be here and save to TempData["OTP"]

                    #region Send OTP
                    string generteOTP = GenerateOTP(6);
                    TempData["OTP"] = generteOTP;
                    string msg = "Hello "+model.FirstName+" "+model.LastName+", Please enter OTP "+ generteOTP+" For Registration";
                    SendSMS(msg, model.Phone);
                    #endregion                    
        ...
        //bhavik
        public virtual ActionResult SendOTP()
        {
            TempData.Keep("customer_email");
        ...
        [PublicAntiForgery]
        [ValidateInput(false)]        
        public virtual ActionResult SendOTP(SendOTPModel model2)
        {            
            var customerEmail = Convert.ToString(TempData["customer_email"]);
            var returnUrl = Convert.ToString(TempData["returnUrl"]);
            var OTP = Convert.ToString(TempData["OTP"]);
            if (!string.IsNullOrEmpty(customerEmail))
            {
                var customer = _customerService.GetCustomerByEmail(customerEmail);
                if (OTP == model2.OTPNumber)
                {
                    //login customer now
                    bool isApproved = _customerSettings.UserRegistrationType == UserRegistrationType.Standard;
                    if (isApproved)
                    {
                        // manually Active customer
                        customer.Active = isApproved;
                        _customerService.UpdateCustomer(customer);
            ...
            TempData.Keep("returnUrl");
            TempData.Keep("OTP");
            model2.OTPNumber = string.Empty;
            ModelState.AddModelError("", _localizationService.GetResource("Account.Register.InvalidOTP"));
            return View(model2);
        }
